...
void RSTPC_T2wrapper::DrawPulsesLenghts()
{
    if (fChain == 0) return;

    Long64_t nEvs = fChain->GetEntriesFast();
...
void RSTPC_T2wrapper::DrawPulsesWfs(Int_t iEv)
{
	if (!fChain) return;
	
	if(!fT1wr->fChain) return;
	
	GetEntry(iEv);
	fT1wr->GetEntry(iEv);
	
	
	Int_t nChs = fT1wr->ColHist->GetNbinsY();
	Int_t nBins = fT1wr->ColHist->GetNbinsX();
	Double_t xlow = -0.5;
	...
	{
		hname.str(""); hname << fT1wr->ColHist->GetName() << "_ch" << iCh;
		colWfsOrig.at(iCh) = fT1wr->ColHist->ProjectionX(hname.str().c_str(), iCh+1, iCh+1);
		...
		hname.str(""); hname << fT1wr->IndHist->GetName() << "_ch" << iCh;
		indWfsOrig.at(iCh) = fT1wr->IndHist->ProjectionX(hname.str().c_str(), iCh+1, iCh+1);
...
void RSTPC_T2wrapper::TestPulsesOverlap(Int_t event, UInt_t colwire, UInt_t indwire)
{//This is a routine mainly used to debug the RSTPC_RunProcessor::CombinePulses(...) routine
	...
	hname.str(""); hname << fT1wr->ColHist->GetName() << "_ch" << colwire;
	ColWfOrig = fT1wr->ColHist->ProjectionX(hname.str().c_str(), colwire+1, colwire+1);
	
	hname.str(""); hname << fT1wr->IndHist->GetName() << "_ch" << indwire;
	IndWfOrig = fT1wr->IndHist->ProjectionX(hname.str().c_str(), indwire+1, indwire+1);
...
void RSTPC_T2wrapper::TestPulsesOverlap(Int_t event)
...
void RSTPC_T2wrapper::PlotColPulses()
{
	if (!fChain) return;
